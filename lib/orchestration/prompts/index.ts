 ...
 * 
 * This comprehensive system prompt is tailored for the ModelForge Blender Agent,
 * inspired by production LLM system prompts (e.g., Claude Code 2.0).
 * 
 * The prompt is structured with XML-like tags for clear section delineation:
 * - <system_role>: Agent identity and purpose
 * - <core_protocol>: Operational guidelines and ReAct reasoning loop
 * - <tools_capability>: Available MCP tools organized by category
 * - <python_scripting_standards>: Best practices for execute_code
 * - <few_shot_examples>: Canonical examples for common workflows
 * - <final_instructions>: Critical behavior constraints
 */

export const BLENDER_AGENT_SYSTEM_PROMPT = `# System Prompt: ModelForge Blender Agent
...
export function buildBlenderSystemPrompt(): string {
    return BLENDER_AGENT_SYSTEM_PROMPT;
}

/**
 * Path to the markdown source file (for documentation reference)
 */
export const SYSTEM_PROMPT_SOURCE = './blender-agent-system.md';
