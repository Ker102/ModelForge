...
export async function similaritySearch(
    ...
    if (options?.source) {
      ...
      SELECT 
        ...
        metadata,
        source,
        1 - (embedding <=> ${`[${queryEmbedding.join(",")}]`}::vector(${EMBEDDING_DIMENSIONS})) as similarity
      FROM document_embeddings
      WHERE source = ${options.source}
        AND 1 - (embedding <=> ${`[${queryEmbedding.join(",")}]`}::vector(${EMBEDDING_DIMENSIONS})) >= ${minSimilarity}
      ORDER BY embedding <=> ${`[${queryEmbedding.join(",")}]`}::vector(${EMBEDDING_DIMENSIONS})
      LIMIT ${limit}
    `
    } else {
        results = await prisma.$queryRaw<SearchResult[]>`
      SELECT 
        ...
        1 - (embedding <=> ${`[${queryEmbedding.join(",")}]`}::vector(${EMBEDDING_DIMENSIONS})) as similarity
      FROM document_embeddings
      WHERE 1 - (embedding <=> ${`[${queryEmbedding.join(",")}]`}::vector(${EMBEDDING_DIMENSIONS})) >= ${minSimilarity}
      ORDER BY embedding <=> ${`[${queryEmbedding.join(",")}]`}::vector(${EMBEDDING_DIMENSIONS})
      LIMIT ${limit}
    `
    }
